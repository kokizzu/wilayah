...
- [2026-08-08]
  ...
  - Replace `hash_equals` with `password_verify` in `login.php` for secure password verification
- [2026-08-06]
  - Fix timing attack vulnerability in `apps/login.php` by using constant-time comparison (`hash_equals`) and type checking
  - Refactor `apps/inc/geo_js.php` to remove unused `$v` variable and empty conditional block
- [2026-08-04]
  - Optimize memory usage in reverse lookup path decoding caching in `apps/inc/reverse_lookup.php`
  - Optimize min/max bounds calculation in `isPathNearCentroid` within `apps/inc/geo_utils.php` by removing `array_column`
  - Fix potential XSS vulnerability in `apps/inc/geo_update.php` JSON responses
  - Remove redundant fallback query in `apps/inc/geo_ajax.php`
- [2026-08-03]
...
- [2026-07-22]
  ...
  - Add test coverage for invalid ring coordinates (empty, 1-point, and 2-point arrays) in `tests/inc/ReverseLookupTest.php`
- [2026-07-21]
  - Optimize `json_decode` in `apps/inc/reverse_lookup.php` with local string dictionary caching
  - Optimize string concatenation using array implode in `index.php`
- [2026-07-20]
